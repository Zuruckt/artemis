...
class ServerRequestFactoryTest extends TestCase
{
    public function test_from_swoole_request_returns_server_request_interface(): void
    {
        ...
        $this->assertEquals('bar', $serverRequest->getQueryParams()['foo'] ?? null);
        $this->assertEquals('qux', $serverRequest->getParsedBody()['baz'] ?? null);
        $this->assertEquals('abc123', $serverRequest->getCookieParams()['session'] ?? null);
    }
}
